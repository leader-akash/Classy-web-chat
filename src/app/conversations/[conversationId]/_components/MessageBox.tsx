import { FullMessageType } from '@/app/_types'
import { useClerk } from '@clerk/nextjs';
import clsx from 'clsx';
import Image from 'next/image';
import React, { useMemo } from 'react'
import {format} from 'date-fns'

interface MessageBoxProps {
    data: FullMessageType;
    isLast?: boolean;

}

const MessageBox = ({data, isLast}: MessageBoxProps) => {
    
    const {user} = useClerk();
    const isOwn = user?.phoneNumbers[0]?.phoneNumber === data?.sender?.phoneNumber

    const seenList = (data.seen || []).filter((user) => user.phoneNumber !== data?.sender?.phoneNumber).join(', ')




    const seenInfo = useMemo(()=>{
        return seenList.length > 0 ? '/images/Seen.svg' : '/images/Sent.svg'
     },[seenList])

    return (
    <div className={clsx('flex mb-[2px] ', isOwn && 'justify-end items-start')}>
      {
        !isOwn && 
        <div className='ml-16 flex items-start '>
            <img src='/images/MessageBubbleTriangleWhite.svg'
            className='transform scale-x-[-1] items-start '
            />
        </div>
      }

      <div className={clsx('flex flex-col', isOwn && 'items-end')}>
        <div className={clsx('text-sm w-fit overflow-hidden', isOwn ? 'bg-[#d1f4cc] text-black' : 'bg-gray-100', data?.image ? 'rounded-[3px]' : 'rounded-[7px] rounded-tr-[0px] py-2 px-3 shadow-lg shadow-gray-300 shadow-bottom')}>
            {
                data?.image ? (
                    <Image 
                        alt="image"
                        height="288"
                        width="288"
                        src={data?.image}
                    />
                )
                :
                (
                    <div className='flex flex-col relative max-w-[600px] '>

                        {/* message text */}
                        <p className='mb-2 break-words mr-20'>{data?.body}</p> 


                        <div className='absolute bottom-0 right-0 flex items-end '>
                            {/* data time */}
                            <div className='text-[11px] h-[15px] text-gray-500'>
                                {format(new Date(data.createdAt), 'p')}
                            </div>

                            {
                                // seen info
                                isOwn  &&  
                                <img src={seenInfo} alt='seen' className='mr-[2px] ml-[4px] w-[16px] h-[11px]' />
                            }
                        </div>

                    </div>
                )
            }
        </div>
            {/* im showing for all including one to one and group if you wanna do in group only we can pass conversation prop */}
            {
                // list of users who have seen message
                isLast && isOwn && seenList?.length > 0  && (
                    <div className='text-xs font-light text-gray-500'>
                        {
                            `Seen by ${seenList}`
                        }
                    </div>
                )
            }
      </div>

            {
                isOwn && 
                <img src="/images/MessageBubbleTriangle.svg" className='mr-16' />
            }

    </div>
  )
}

export default MessageBox
